...
const SeeEstimatedBudget = () => {
    ...
    const handleApprove = async () => {
        try {
            const response = await axios.put(`https://api.blueace.co.in/api/v1/update-status-bills/${estimatedBill._id}`, {
                status: true
            });
    ...
    const handleDecline = async () => {
        try {
            const response = await axios.put(`https://api.blueace.co.in/api/v1/update-order-status/${estimatedBill._id}`, {
                status: false
            });
            if (response.status === 200) {
