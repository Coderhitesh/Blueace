...
function Forget() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post("http://localhost:7000/api/v1/Password-Change", formData)
  ...
  const handleOTPSubmit = async (otpevent) => {
    ...
    try {
      const response = await axios.post(`http://localhost:7000/api/v1/Verify-Otp`, formData)
      console.log(response.data);
      // setLoading(false);
      toast.success(response.data.msg)
  ...
  const resendOTP = async (otpevent) => {
    ...
    otpevent.preventDefault()
    try {
      const response = await axios.post(`http://localhost:7000/api/v1/resend-otp/`, formData)
      console.log(response.data);
      // setLoading(false);
