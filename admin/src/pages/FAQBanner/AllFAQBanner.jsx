...
function AllFAQBanner() {
    ...
    const handleFetchData = async () => {
        setLoading(true);
        try {
            const response = await axios.get('http://localhost:7000/api/v1/get-all-faq-banner');
    ...
    const handleDelete = async (id) => {
        try {
            const response = await axios.delete(`http://localhost:7000/api/v1/delete-faq-banner/${id}`);
            if (response.data.success) {
    ...
    const handleToggle = async (id, currentActiveStatus) => {
        try {
            ...
            const response = await axios.put(`http://localhost:7000/api/v1/update-faq-banner-active-status/${id}`, {
