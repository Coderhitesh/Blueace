...
function EditFAQContent() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const { data } = await axios.get(`http://localhost:7000/api/v1/get-single-faq-content/${id}`)
                const faq = data.data;
                setFormData({
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.put(`http://localhost:7000/api/v1/update-faq-content/${id}`, payload, {
                headers: {
