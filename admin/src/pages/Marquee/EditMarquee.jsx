...
function EditMarquee() {
    ...
    useEffect(() => {
        const fetchSingleData = async () => {
            try {
                const { data } = await axios.get(`https://api.blueace.co.in/api/v1/get-single-marquee/${id}`)
                const res = data.data;
                setFormData({
                    text: res.text
                })
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.put(`https://api.blueace.co.in/api/v1/update-marquee/${id}`, payload, {
                headers: {
